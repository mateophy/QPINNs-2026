...
def complex_mse_loss_magnitude(input_complex, target_complex):
        ...
        loss = torch.mean(complex_diff * complex_diff.conj()).to(torch.float)
        return loss

# def complex_mse_loss_magnitude(input_complex, target_complex):
#         # Calculate MSE for the real parts
#         mse_real = torch.nn.functional.mse_loss(input_complex.real, target_complex.real)
#         # Calculate MSE for the imaginary parts
#         mse_imag = torch.nn.functional.mse_loss(input_complex.imag, target_complex.imag)
#         # Sum the squared errors from real and imaginary parts
...
class ClassicalSolver2(nn.Module):
    def __init__(self, args, logger: Logging, data=None, device=None):
        ...
        # ARL: change of preprocessor types
        ...
        self.preprocessor = nn.Sequential(
            ...
            nn.Tanh().to(torch.cfloat),
            nn.Linear(self.classic_network[-2], self.classic_network[-2]).to(
                self.device
            ).to(torch.cfloat),
        ).to(self.device)

        # ALR: Change of hidden types
        ...
        # ALR: Change of postprocessor types
        ...
        self.postprocessor = nn.Sequential(
            nn.Linear(self.classic_network[-2], self.classic_network[-2]).to(
                self.device
            ).to(torch.cfloat),  # 2* for X and P quadratures
            nn.Tanh().to(torch.cfloat),
            nn.Linear(self.classic_network[-2], self.classic_network[-1]).to(
                self.device
            ).to(torch.cfloat),
        ...
        # ALR: Change of activation type
        # self.activation = nn.Tanh()

        self.activation = nn.Tanh().to(torch.cfloat)
